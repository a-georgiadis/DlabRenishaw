...
class BatchMappingWidget(QWidget):
    def __init__(self, Renishaw:RenishawAdapter):
        super().__init__()
        
        # Connection to Renishaw
        self.renishaw_adapter = Renishaw
    ...
    def load_settings_file(self):
        path, _ = QFileDialog.getOpenFileName(self, "Load Template", self.line_filepath.text(), "WXM Files (*.wxm);;All Files (*)")
        if path:
    ...
    def _get_current_stage_position(self):
        """
        Currently Hooked Up Directly to Renishaw Microscope
        """
        # Returning dummy data for GUI testing
        return self.renishaw_adapter.stage.get_position()

    def _send_move_command(self, x, y, z):
        """
        Hooked Up Directly to Renishaw Microscope
        """
        self.renishaw_adapter.stage.move_to(x,y,z)
